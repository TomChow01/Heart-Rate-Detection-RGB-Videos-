...
class Face_utilities():
    ...
    def __init__(self, face_width = 200, detector = None):
...
#        if self.detector == 'dlib':
#            print("Using dlib face detector(1)")
...
#        self.detector = cv2.dnn.readNetFromCaffe(prototxt_path, caffemodel_path)
        ...
        for i in range(0, 1):
            ...
            # prediction
            #print("det: ", detections[0,0,i,:])
            confidence = detections[0, 0, i, 2]
            #print("conf: ", detections[0, 0, 0, 2])
            ...
            if confidence > 0.5:
                ...
                rects = dlib.rectangle(int(startX), int(startY), int(endX), int(endY))
                #rects = (int(startX), int(startY), int(endX-startX), int(endY-startY))
                #self.detector = None
            else:
                rects = None
#        else:
...
#            rects = self.detector(gray, 1)
    ...
    def get_landmarks(self, frame, type):
        ...
        if self.predictor is None:
            ...
            self.predictor = dlib.shape_predictor('/gdrive/MyDrive/code/Heart Rate Detection/packages/shape_predictor_68_face_landmarks.dat')
            #self.predictor = dlib.shape_predictor("/gdrive/MyDrive/code/Heart Rate Detection/packages/shape_predictor_" + type + "_face_landmarks.dat")
        ...
        if rects is None:
            return None, None
        rects_ = face_utils.rect_to_bb(rects)
        if len(rects_)<0 or len(rects_)==0:
            return None, None
            
#        shape = self.predictor(gray, rects[0])
        shape = self.predictor(gray, rects)
